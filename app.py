...
def package_details(name):
    ...
    l.extend([stm.id, stm.title, stm.price, stm.location,
             stm.img, stm.subheading, stm.body])
...
def booking(pid):
    ...
    if 'f_name' in request.form and 'l_name' in request.form and 'phone' in request.form and 'email' in request.form and 'dep_date' in request.form and 'arrive_date' in request.form and 'count' in request.form and 'id' in request.form:
        ...
        book = booking(first_name=f_name, last_name=l_name, people_count=count, email=email, phone=phone,
                       package_title=row.title, period=row.subheading, dep_date=dep_date, arrival_date=arrive_date, price=price)
